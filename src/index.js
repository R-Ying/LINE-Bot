...
submitButton.addEventListener("click", async () => {
    ...
    try {
        ...
        if (subcategory === 'IRI 平坦度' || subcategory === 'iri') {
            ...
            if (!recordedData || !recordedData.accelerometer || recordedData.accelerometer.length === 0) {
                alert('請先進行路面平坦度測量');
                return;
            }
            ...
            Object.entries({
                ...
                extraDetails: `總距離: ${recordedData.totalDistance.toFixed(2)}米, 最終ARI: ${recordedData.ARI ? recordedData.ARI.toFixed(4) : '未計算'}`,
                latitude: lastGPS.latitude.toString(),
                longitude: lastGPS.longitude.toString()
            }).forEach(([key, value]) => data.append(key, value));

            if (recordedData.badRoadSegments?.length > 0) {
                data.append("ariData", JSON.stringify({
            ...
            await new Promise((resolve) => {
                EXIF.getData(file, function () {
                    ...
                    data.append("latitude", location.latitude ? location.latitude.toString() : "");
                    data.append("longitude", location.longitude ? location.longitude.toString() : "");
                    resolve();
                });
            });
...
function resetForm() {
    ...
    if (document.getElementById('accelerometerData')) {
        document.getElementById('accelerometerData').textContent = '加速度計：等待數據...';
    }
    ...
    if (document.getElementById('distanceData')) {
        document.getElementById('distanceData').textContent = '總行駛距離：0.00 米';
    }
  ...
  function populateNewFormOptions(subcategory) {
    const detailOptionsContainer = document.getElementById('detailOptionsContainer');
    ...
    if (subcategory === 'iri') {
        ...
        if (!recordingControls) {
            ...
            subcategoryContainer.parentNode.insertBefore(recordingDiv, subcategoryContainer.nextSibling);
  ...
  // 在 index.js 中修改 submitLocation 函數
function submitLocation() {
  const userId = document.getElementById("detailsInput").value;
  ...
  const newFormSelect = document.getElementById("newFormSelect");
  const detailOptionsSelect = document.getElementById("detailOptionsSelect");
  const extraDetailsInput = document.getElementById("extraDetailsInput");

  formData.append("category", categoryTranslations[categorySelect.value]);
  formData.append("subcategory", subcategoryTranslations[subcategorySelect.value]);
  formData.append("newFormOption", newFormSelect.value);
  formData.append("detailOption", detailOptionsSelect.value);
  formData.append("extraDetails", extraDetailsInput?.value || "");
...
function updateLiveData(data) {
  ...
  if (data.ARI !== null && data.ARI !== undefined) {
      document.getElementById('ariData').textContent = `ARI：${data.ARI.toFixed(4)}`;
      // 根據 ARI 值添加不同的顏色
